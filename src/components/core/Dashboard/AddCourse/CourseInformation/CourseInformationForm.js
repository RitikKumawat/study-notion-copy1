...
export const CourseInformationForm = () => {
    ...
    const isFormUpdated = ()=>{
        ...
        if(getCurrentValues.courseTitle !== course.courseName ||
            getCurrentValues.courseShortDesc !== course.courseDescription ||
            getCurrentValues.coursePrice !== course.price ||
            getCurrentValues.courseTags.toString() !== course.tag.toString() ||
            getCurrentValues.courseBenefits !== course.whatYouWillLearn ||
            getCurrentValues.courseCategory._id !== course.category._id ||
            getCurrentValues.courseImage !== course.thumbnail ||
            getCurrentValues.courseRequirements.toString() !== course.instructions.toString())
        return true;
        else
        return false;
    }
    const onSubmit = async(data)=>{
        ...
        formData.append("whatyouwilllearn",data.courseBenefits);
        formData.append("category",data.courseCategory);
        formData.append("istructions",JSON.stringify(data.courseRequirements));
    ...
    className='rounded-md border-[1px] border-richblack-700 bg-richblack-800 p-6 space-y-8'>
        ...
        <div className='flex flex-col space-y-2'>
            ...
            <select id='courseCategory' className='form-style w-full'
            defaultValue=""
